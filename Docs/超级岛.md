...
> 更新时间：2026-01-29 10:51:01
> 来源：[小米澎湃OS开发者平台](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=2131)
...
数据具体字段及含义请参见下文：**基础数据构建**，不同模板有不同的摘要态大小岛和焦点通知数据字段，详情可以查看**模板库**说明。
...
关于 `miui.focus.param` 和各个模板的字段及含义，请查看 **交互能力设计** 和 **模版库** 部分。
...
- 通知单张图片的大小不得超过 100k，超过 100k 则不下载，视为下载失败；
- 图片下载全流程最大等待 180s，超时则下载失败；
- 通知图片链接必须是 Https 格式，否则按下载失败处理；
- 图片宽高比在 1.78(16:9) - 1(1:1) 之间；
- 单个通知图片的数量不得大于 10 张，若超过 6 张，XMSF 只会按顺序下载 10 张，其他不下载。
...
| islandFirstFloat | 否 | boolean | 通知第一次出现时，是否自动显示展开态<br>- true: 展开态<br>- false: 摘要态<br>默认值：true |
| enableFloat | 否 | boolean | 通知更新时，是否自动展开为展开态<br>- true: 展开<br>- false: 不展开<br>默认值：false |
| timeout | 否 | int | 通知的默认消失时间<br>- 单位：min<br>- 默认值：720 |
| cancel | 否 | boolean | 是否直接结束通知<br>针对 mipush，值为 true 时，触发移除通知 |
| updatable | 否 | boolean | 是否为持续性通知<br>- 默认: false |
| reopen | 否 | String | 相同 notification id 的通知被 cancel 后，再次发送是否显示，前提：update = true<br>- reopen: 再次显示<br>- close: 不显示<br>- 默认: close |
| filterWhenNoPermission | 否 | boolean | 应用焦点通知权限被关闭时退化为普通通知的处理方式：<br>- false，通知正常显示不被过滤<br>- true，通知后被过滤，不显示<br>- 默认值：false |
| orderId | 否 | String | 订单号，MIPUSH 字段 |
| sequence | 否 | long | 本订单第几次更新通知，避免通知展示乱序。（MIPUSH 实时更新类必传） |
...
通知内容数据同时在焦点通知和岛展开态生效，不同模板显示内容不同，具体字段请查看[《小米超级岛模板库》](api\小米超级岛通知模板库.md)。

### 3、岛属性数据

| 参数 | 必选 | 参数类型 | 含义 |
| --- | --- | --- | --- |
| islandProperty | 否 | int | 岛属性<br>- 1：信息展示为主<br>- 2：操作为主<br>- 默认值: 1 |
| islandOrder | 否 | boolean | 通知更新时，但摘要态隐藏时，是否更新岛排序：<br>- true: 更新排序<br>- false: 不更新<br>- 默认值：false |
| islandTimeout | 否 | int | 岛自动消失的时间<br>- 单位：s<br>- 默认值: 60 * 60 |
| dismissIsland | 否 | boolean | 摘要态消失<br>- true：消失<br>- false: 不消失 |
...
| aodPic | 否 | String | 息屏模式图标<br>不传默认为应用图标，<br>自定义图标需要和 miui.focus.pics 参数对应 |
...
| tickerPic | 否 | String | OS2 焦点通知状态栏图标<br>不传默认为应用图标，<br>自定义图标需要和 miui.focus.pics 参数对应 |
...
调用 android 原生接口，cancel 通知即可。

参考链接：[https://developer.android.google.cn/reference/kotlin/android/app/NotificationManager?hl=en](https://developer.android.google.cn/reference/kotlin/android/app/NotificationManager?hl=en)
...
OS2 和 OS3 支持焦点通知模版不同，同时只在 OS3 版本上支持岛，开发者可以根据查询版本结果，有选择的选择接入 OS2 焦点通知或者 OS3 岛通知。
...
- 具体接入请见：[《Xiaomi HyperOS 媒体通知适配说明》](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=2161)
- 拖拽分享能力需单独适配，详情请见：[《媒体通知岛上拖拽分享适配说明》](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=2162)
...
除了使用原生 `Notification.Action`，也可以通过 `ActionInfo` 来自定义构建按钮点击效果，具体可以查看模板库中 `ActionInfo` 字段相关含义。
