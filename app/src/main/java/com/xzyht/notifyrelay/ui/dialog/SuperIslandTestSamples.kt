 ...
 * 超级岛测试样例数据与触发入口。
 *
 * 从 SuperIslandTestDialog 拆出：样例构造占据数百行且与对话框 UI 无关，
 * 混在同一文件会让该文件过大、难以维护。此处集中：
 * - showTestNotification：按「发送到其他设备」开关选择远端发送或本地浮窗显示；
 * - 各 testXxx 样例构造（internal，供对话框按钮与 triggerSuperIslandTestSample 调用）；
 * - triggerSuperIslandTestSample：按样例 ID 触发的调试入口（供 adb / UI 复用）。
 *
 * 注意：样例中的时间戳必须取当前时间。曾因写死历史时间戳导致系统
 * FocusNotifPreHandler 将通知判为旧包（filterOut old）而直接拦截投递。
 */

// 递增进度计数器，供 getProgress 在「进度可变」模式下循环取值
 ...
 * TODO：数据并不符合超级岛规范
 */
internal fun testBaseInfo(context: Context) {
    val paramV2Raw = """
        {
            "protocol": 1,
 ...
 * 测试IM图文组件
 */
internal fun testChatInfo(context: Context) {
    val paramV2Raw = """
        {
            "chatInfo": {
                "title": "聊天测试",
 ...
 * 测试动画文本组件
 */
internal fun testAnimTextInfo(context: Context) {
    val paramV2Raw = """
        {
            "animTextInfo": {
                "title": "动画文本测试",
 ...
 * 测试强调图文组件
 */
internal fun testHighlightInfo(context: Context) {
    val paramV2Raw = """
        {
            "highlightInfo": {
                "title": "强调文本测试",
 ...
 * 测试识别图形组件
 */
internal fun testPicInfo(context: Context) {
    val paramV2Raw = """
        {
            "picInfo": {
                "title": "图形测试",
 ...
 * 测试提示组件
 */
internal fun testHintInfo(context: Context) {
    val paramV2Raw = """
        {
            "hintInfo": {
                "title": "提示测试",
 ...
 * 测试文本按钮组件
 */
internal fun testTextButton(context: Context) {
    val paramV2Raw = """
        {
            "protocol": 1,
 ...
 * 测试多节点进度组件
 */
internal fun testMultiProgressInfo(
    context: Context,
    isVariableProgress: Boolean,
) {
    // 获取进度值
 ...
 * 测试带有图标的多节点进度组件
 */
internal fun testMultiProgressWithIcons(
    context: Context,
    isVariableProgress: Boolean,
) {
    // 获取进度值
 ...
 * 测试圆形进度组件（基于真实小米互传数据）
 */
internal fun testCircularProgressInfo(
    context: Context,
    isVariableProgress: Boolean,
) {
    // 获取进度值
